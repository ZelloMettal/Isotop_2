...
namespace Isotop2.Data.Controllers
{
    internal class TechnetiumPrintController
    {
        ...
        static public async Task PrintDocument(bool isPrintAdult, ListView lv_Adult, bool isPrintChildren, ListView lv_Children)
        {
            if (isPrintAdult && !isPrintChildren)
            {
                ...
                await Task.Run(() => _model.CreateOneTable(data, countRows, "Взрослые"));
            }
            else if (!isPrintAdult && isPrintChildren)
            {
                ...
                await Task.Run(() =>  _model.CreateOneTable(data, countRows, "Дети"));
            }
            else if (isPrintAdult && isPrintChildren)
            {
                //Получаем саписок данных для печати
                ...
                await Task.Run(() => _model.CreateTwoTable(dataAdult, countRowsAdult, dataChildren, countRowsChildren));
            }
        }
    }
}
