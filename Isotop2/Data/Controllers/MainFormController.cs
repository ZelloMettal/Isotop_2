...
namespace Isotop2.Data.Controllers
{
    internal class MainFormController
    {
        ...
        //Метод печати Йода
        static public async Task PrintIodine(ListView lv, decimal activity)
        {
            ...
            await Task.Run(() => IodinePrintController.CreateIodineTable());
        }
        ...
        //Метод печати Радия
        static public async Task PrintRadium(ListView patientList, DateTime currentDate, decimal weightPatient, double activity)
        {
            List<string[]> dataList = _model.GetRadiumForPatient(weightPatient, activity);
            RadiumPrintController.SetPrintData(patientList, currentDate, dataList.First()[0], dataList.First()[1], dataList.First()[2]);
            await Task.Run(() => RadiumPrintController.CreateRadiumTable());
        }
